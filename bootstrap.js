...
const self = {
	name: 'ProfilistPortable',
	chrome_path: 'chrome://profilistportable/content/',
	aData: 0,
};
		...
		if (!isPortable) {
			...
			var checkReadyAndLaunch = function() {
				...
				if (profilesIniUpdateDone && rootDirMakeDirDone && localDirMakeDirDone) {
					myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'Creating Profile', 'Default Name: "' + profName + '"', false, null, null, 'ProfilistPortable');
		...
		if (!selectedProfileNameFound) {
			...
			if (!profToolkit.selectedProfile.name) {
				...
				if (profilistLoadingT) {
					...
					return new Error('Using Temporary Profile - ProfilistPortable will not work');
				}
			}
...
function updateMenuDOM(aDOMWindow, json, jsonStackChanged, dontUpdateDom) {
	...
	for (var i=0; i<json.length; i++) {
		...
		if (elHeight == 0) {
			myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'DEBUG', 'elHeight = 0', false, null, null, 'ProfilistPortable');
			if (appendChild) {
				...
				if (confirmCheck.value) {
					...
					promise.then(
						function() {
							...
							myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'Profile Deleted', 'The profile "' + oldProfName +'" was succesfully deleted.', false, null, null, 'ProfilistPortable');
						},
						function(aRejectReason) {
			...
			if (newProfName != oldProfName) {
				...
				promise.then(
					function() {
						...
						myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'Profile Renamed', 'The profile "' + oldProfName +'" was succesfully renamed to "' + newProfName + '"', false, null, null, 'ProfilistPortable');
						updateProfToolkit(1, 1);
					},
					function(aRejectReason) {
...
function launchProfile(e, profName, suppressAlert, url) {
	...
	if (!suppressAlert) {
		myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'Launching Profile', 'Profile Name: "' + profName + '"', false, null, null, 'ProfilistPortable');
	}
...
var windowListener = {
	...
	loadIntoWindow: function (aDOMWindow) {
		...
		if (PanelUI) {			
			...
			//myServices.as.showAlertNotification(self.aData.resourceURI.asciiSpec + 'icon.png', self.name + ' - ' + 'DEBUG', 'PUIsync_height set to = ' + PUIsync_height, false, null, null, 'ProfilistPortable');
